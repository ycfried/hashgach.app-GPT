...
export default async function ProtectedApp(){
  ...
  const [studentResult,periodResult,classResult,staffResult,offeringResult,assignmentResult,sessionResult,typeResult,disciplineResult,schoolResult,zmanResult,testResult,gradeResult,bankResult,transferResult,scheduleTemplateResult,scheduleBlockResult,scheduleInstanceResult,mentorAssignmentResult,conversationResult,noteRequestResult,statsRequestResult,statsSessionResult,statsAttendanceResult,reportCardResult,chatResult,auditResult]=await Promise.all([
    ...
    supabase.from("staff").select("id,name,roles").order("name"),
    ...
    supabase.from("schools").select("name,settings").eq("id",staff.school_id).maybeSingle(),
    ...
    supabase.from("attendance_records").select("attendance_session_id,student_id,status,late_minutes"),
    supabase.from("report_cards").select("id,student_id,zman_id,range_start,range_end,generated_at,status,snapshot").order("generated_at",{ascending:false}),
    supabase.from("chat_messages").select("id,sender_id,channel_type,channel_id,body,created_at").order("created_at").limit(500),
    supabase.from("audit_log").select("id,actor_id,action,entity_type,entity_id,before_value,after_value,occurred_at").order("occurred_at",{ascending:false}).limit(500),
  ]);
  ...
  const reportsData:ReportsBundle={students:students.filter(s=>s.id).map(s=>({id:s.id!,name:s.name,year:s.year})),zmanim:zmanResult.data||[],sessions:statsSessionResult.data||[],attendance:statsAttendanceResult.data||[],tests:(testResult.data||[]).map(t=>({id:t.id,name:t.name,test_date:t.test_date,zman_id:t.zman_id,test_type:t.test_type})),grades:(gradeResult.data||[]).map(g=>({student_id:g.student_id,test_id:g.test_id,raw_score:g.raw_score,applied_bank:g.applied_bank,final_score:g.final_score})),discipline:(disciplineResult.data||[]).map(d=>({student_id:d.student_id,status:d.status,current_amount:d.current_amount})),conversations:(conversationResult.data||[]).map(c=>({student_id:c.student_id,conversation_date:c.conversation_date})),reportCards:(reportCardResult.data||[]) as ReportsBundle["reportCards"]};
  const chatData:ChatBundle={staff:(staffResult.data||[]).map(s=>({id:s.id,name:s.name})),messages:(chatResult.data||[]) as ChatBundle["messages"]};
  const adminData:AdminBundle={schoolName:schoolResult.data?.name||"",settings:(schoolResult.data?.settings||{}) as Record<string,unknown>,staff:(staffResult.data||[]).map(s=>({id:s.id,name:s.name})),audit:(auditResult.data||[]) as AdminBundle["audit"],students:(studentResult.data||[]) as Record<string,unknown>[],grades:(gradeResult.data||[]) as Record<string,unknown>[],attendance:(statsAttendanceResult.data||[]) as Record<string,unknown>[],discipline:(disciplineResult.data||[]) as Record<string,unknown>[]};
  return <AppShell profileName={staff.name} roles={staff.roles||[]} schoolId={staff.school_id} userId={user.id} initialStudents={students} setupData={setupData} attendanceData={attendanceData} disciplineData={disciplineData} gradesData={gradesData} scheduleData={scheduleData} mentoringData={mentoringData} reportsData={reportsData} chatData={chatData} adminData={adminData}/>;
}
