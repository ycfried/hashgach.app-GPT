...
export default function LoginPage() {
  ...
  async function submit(event:FormEvent){event.preventDefault();setBusy(true);setError("");const supabase=createClient();const {data,error}=await supabase.auth.signInWithPassword({email,password});if(error){setError(error.message);setBusy(false);return}const pending=data.user.user_metadata?.pending_invite;const name=data.user.user_metadata?.signup_name;if(pending&&name){const result=await supabase.rpc("accept_staff_invite",{p_token:pending,p_name:name});if(result.error&&!result.error.message.includes("invalid or expired")){setError(`Your account was confirmed, but staff access could not be completed: ${result.error.message}`);setBusy(false);return}}router.push(new URLSearchParams(location.search).get("next")||"/app");router.refresh()}
