...
export default function AttendanceView({ schoolId, userId, isPrincipal, initial }: { schoolId: string; userId: string; isPrincipal: boolean; initial: AttendanceBundle }) {
  ...
  if (offering && session?.status === "active") return <><div className="page-title"><div><h1>Live attendance</h1><p>Click once for on time. Double-click to record the student’s arrival right now.</p></div></div>{error && <p className="form-error attendance-error">{error}</p>}<div className="card live-roster"><div className="live-head"><div><span className="live-label"><i /> LIVE CLASS</span><h2>{offering.className}</h2><p>{offering.periodName} · Scheduled {time(offering.startTime)} · Started {time(session.startTime)} · {roster.length} students</p></div><button className="secondary" disabled={busy} onClick={complete}>Complete class</button></div><button className="mark-all" disabled={busy || !roster.length} onClick={markAll}><Check /> Mark all present</button>{roster.length ? roster.map((student) => { const record = session.records.find((r) => r.studentId === student.id); const excusal = applicableExcusal(student.id, offering.id); return <Fragment key={student.id}><div className="roster-row"><div className="person"><span className="avatar small">{student.name.split(" ").map((n) => n[0]).join("")}</span><span><b>{student.name}</b>{record && <small className={`attendance-result ${record.status}`}>{labels[record.status]}{record.status === "late" ? ` · ${record.lateMinutes} min late` : ""}</small>}{excusal && <small>Principal excusal{excusal.reason ? ` · ${excusal.reason}` : ""}{excusal.end_date ? ` · through ${new Date(`${excusal.end_date}T12:00:00`).toLocaleDateString()}` : " · ongoing"}</small>}</span></div><div className="arrival-actions"><button className={`arrival-button ${record?.status || ""}`} disabled={savingStudent === student.id || !!excusal} onClick={() => checkIn(student.id)} onDoubleClick={() => checkInNow(student.id)} aria-label={`Check in ${student.name}. Click once for on time; double-click for arrival now.`}>{savingStudent === student.id ? "Saving…" : record ? <><Check /> {record.status === "late" ? `${record.lateMinutes}m late` : labels[record.status]}</> : <><Check /> Check in</>}<small>1× on time · 2× arrival now</small></button><details className="attendance-more"><summary aria-label={`More attendance choices for ${student.name}`}><ChevronDown /></summary><div className="segmented">{options.map((option) => <button key={option} className={record?.status === option ? `selected ${option}` : ""} disabled={savingStudent === student.id} onClick={() => mark(student.id, option)}>{labels[option]}{option === "late" && record?.status === "late" ? ` · ${record.lateMinutes}m` : ""}</button>)}</div></details></div></div>{lateTarget === student.id && <section className="late-inline-editor" aria-label={`Correct late arrival for ${student.name}`}><div><span className="eyebrow">Late arrival</span><b>{student.name}</b><small>Enter the exact number of minutes late.</small></div><label>Minutes late<input type="number" min="0" autoFocus value={lateValue} onChange={(e) => setLateValue(e.target.value)} /></label><div className="inline-editor-actions"><button className="secondary" onClick={() => setLateTarget(null)}>Cancel</button><button className="primary" onClick={() => persistMark(student.id, "late", Math.max(0, Number(lateValue) || 0))}>Save correction</button></div></section>}</Fragment>; }) : <div className="empty-state"><Users /><p>No students are assigned to this period yet. Add them under Setup → Assignments.</p></div>}</div></>;
